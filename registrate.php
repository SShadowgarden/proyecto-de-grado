...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (empty($usuario)  or empty($password) or empty($password2)) {
		...
		$statement = $conexion->prepare('SELECT * FROM datos WHERE Nombre = :usuario LIMIT 1');
	...
	if ($errores == '') {
		$statement = $conexion->prepare('INSERT INTO datos (ID, Nombre,  Contrasena) VALUES (null,:usuario,:pass)');
